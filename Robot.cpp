...
void Robot::wanderAvoidingCollisions()
{
    ...
    float mode = rand() % 100;
    float sign = rand () % 2;
    float safeDistance = 0.5;

    // ((minLeftSonar < 0.35) && (minFrontSonar < 0.35)) || ((minRightSonar < 0.35) && (minFrontSonar < 0.35)) || ((minLeftSonar < 0.35) && (minRightSonar < 0.35) && (minFrontSonar < 0.35))
    if(minFrontSonar < safeDistance || minFrontLaser < safeDistance){
        std::cout << "Warning: getting close to a wall!" << std::endl;

        //std::cout << minLeftSonar << std::endl;
        //std::cout << minRightSonar << std::endl;
        //std::cout << (fabs(minLeftSonar - minRightSonar)) << std::endl;

        if(((minLeftSonar > minRightSonar) || (minLeftLaser > minRightLaser)) && (fabs(minLeftLaser - minRightLaser)) > 0.1 ){
            std::cout << "Rotate left!" << std::endl;
            linVel = 0;
            angVel = 45;
        }
        else if(((minLeftSonar < minRightSonar) || (minLeftLaser < minRightLaser)) && (fabs(minLeftLaser - minRightLaser)) > 0.1 ){
            ...
            // Robot cornered!
            std::cout << "Rotate reverse!" << std::endl;
            linVel = 0;
            angVel = 180;
        }
    }
        ...
        if(mode < 75){
            ...
            //std::cout << "Rotating! angVel = " << angVel << std::endl;
         }
    }
